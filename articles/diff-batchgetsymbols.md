...
- All input arguments are now formatted as “snake_case” and not
  ...
  importation in
  [`yfR::yf_get()`](https://docs.ropensci.org/yfR/reference/yf_get.md)
  is `first_date`, and not `first.date`, as used in
  `BatchGetSymbols::BatchGetSymbols`.

- A new feature called “collection”, which allows for easy download of a
  collection of tickers. For example, you can download price data for
...
- All function have been renamed for a common API notation. For example,
  `BatchGetSymbols::BatchGetSymbols` is now
  [`yfR::yf_get()`](https://docs.ropensci.org/yfR/reference/yf_get.md).
  Likewise, the function for fetching collections is
  [`yfR::yf_collection_get()`](https://docs.ropensci.org/yfR/reference/yf_collection_get.md).

- The output of
  [`yfR::yf_get()`](https://docs.ropensci.org/yfR/reference/yf_get.md)
  is always a tibble with the price data (and not a list as in
  `BatchGetSymbols::BatchGetSymbols`). If one wants the tibble with a
  summary of the importing process, it is available as an attribute of
  the output (see function
